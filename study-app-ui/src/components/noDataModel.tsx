...
function NoDataModel({ title = "No Data found", className }: Props) {
  return (
    <div className={`pt-8 grid place-items-center ${className}`}>
      <div className="shadow-custom flex flex-col gap-y-4 items-center py-16 w-full max-w-[720px] rounded-2xl">
        ...
        <span className="text-2xl px-8 text-center text-slate-400">{title}</span>
        <Link to={routers.home} className="btn-custom mt-4">Go home</Link>
      </div>
    </div>
  );
}
