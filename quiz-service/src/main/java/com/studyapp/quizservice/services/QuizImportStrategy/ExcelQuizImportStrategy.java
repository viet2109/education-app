...
public class ExcelQuizImportStrategy implements QuizImportStrategy {
    ...
    public QuizResponseDto convertFileToDto(MultipartFile multipartFile, String userId) {
        ...
        try (Workbook workbook = new XSSFWorkbook(multipartFile.getInputStream())) {
            ...
            // Đọc dòng dữ liệu đầu tiên (sau header)
            ...
            if (rowQuiz != null && !isRowEmpty(rowQuiz)) {
                ...
                // Kiểm tra và gán giá trị cho QuizEntity
                if (!title.isEmpty()) {
                    quiz.setTitle(title);
                }
                ...
                // Lưu quiz vào danh sách hoặc cơ sở dữ liệu
            ...
            // Serialize and send data to QuestionClient
        ...
        for (XSSFShape shape : drawing.getShapes()) {
            if (shape instanceof XSSFPicture picture) {
                ...
                if (anchor.getRow1() == address.getRow() && anchor.getCol1() == address.getColumn()) {
                    ...
                    String filePath = pictureData.getPackagePart().getPartName().getName();
